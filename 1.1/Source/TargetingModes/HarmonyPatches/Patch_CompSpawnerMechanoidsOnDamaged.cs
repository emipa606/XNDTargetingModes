    ...
    public static class Patch_CompSpawnerMechanoidsOnDamaged
    {
        ...
        public static class Patch_TrySpawnMechanoids
        {
            ...
            private static bool IsTheInstructionIAmLookingFor(CodeInstruction instruction, List<CodeInstruction> instructionList, int i)
            {
                ...
                //instruction.operand == AccessTools.Field(typeof(CompSpawnerMechanoidsOnDamaged), nameof(CompSpawnerMechanoidsOnDamaged.pointsLeft)) && 
                bool result = instruction.opcode == OpCodes.Stfld && 
                    prevInst.opcode == OpCodes.Sub && prevInst2.opcode == OpCodes.Ldfld && (FieldInfo)prevInst2.operand == AccessTools.Field(typeof(PawnKindDef), nameof(PawnKindDef.combatPower));
                return result;
